...
function replaceOnce(source, before, after, label) {
  ...
  if (first < 0) throw new Error(`Manual-calibration-v2 target missing: ${label}`);
  if (source.indexOf(before, first + before.length) >= 0) {
    throw new Error(`Manual-calibration-v2 target is ambiguous: ${label}`);
  }
...
function insertAfterLineContaining(source, needle, insertion, label) {
  ...
  if (first < 0) throw new Error(`Manual-calibration-v2 line missing: ${label}`);
  if (source.indexOf(needle, first + needle.length) >= 0) {
    throw new Error(`Manual-calibration-v2 line is ambiguous: ${label}`);
  }
  const lineEnd = source.indexOf('\n', first);
  if (lineEnd < 0) throw new Error(`Manual-calibration-v2 line ending missing: ${label}`);
...
function patchFinalPose(source) {
  ...
  if (start < 0) throw new Error('Manual-calibration-v2 final pose block missing');
  if (source.indexOf(marker, start + marker.length) >= 0) {
    throw new Error('Manual-calibration-v2 final pose block is ambiguous');
  }
  const assignment = 'poseRef.current = pose;';
  const assignmentAt = source.indexOf(assignment, start);
  if (assignmentAt < 0) throw new Error('Manual-calibration-v2 pose assignment missing');
...
function patchOverlayMarkers(source) {
  ...
  if (at < 0) throw new Error('Manual-calibration-v2 overlay boundary missing');
  if (source.indexOf(marker, at + marker.length) >= 0) {
    throw new Error('Manual-calibration-v2 overlay boundary is ambiguous');
  }
...
`;
  center = replaceOnce(
    ...
    `${handlers}  const evidenceReady = canCountGuidedStrum(vision);`,
    'manual handlers',
  );
...
console.log('Connected manual three-point calibration at the final adaptive pose boundary.');
