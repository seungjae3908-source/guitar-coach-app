...
function hand(centerX, centerY, scale) {
...
{
    const decision = (0, hand_precision_region_1.decideHandPrecisionRegion)({ hasHand: true, landmarks: hand(0.52, 0.54, 0.14) });
...
}
{
    ...
    const decision = (0, hand_precision_region_1.decideHandPrecisionRegion)({ hasHand: true, landmarks });
...
{
    const decision = (0, hand_precision_region_1.decideHandPrecisionRegion)({ hasHand: true, landmarks: hand(0.5, 0.5, 0.44) });
    assert(!decision.shouldRefine, '이미 화면에 크게 보이는 손을 다시 과도하게 확대하면 안 됩니다.');
}
{
    const decision = (0, hand_precision_region_1.decideHandPrecisionRegion)({ hasHand: true, landmarks: hand(0.5, 0.5, 0.12).slice(0, 10) });
...
}
{
    ...
    const effective = (0, hand_precision_region_1.effectiveHandDetailSize)({ landmarks, precision: { applied: true, region: { left: 0.35, top: 0.34, right: 0.65, bottom: 0.66 } } });
    assert(effective > 0.17, 'ROI 정밀 분석은 원본 좌표가 아니라 ROI 내부의 유효 손 크기로 평가해야 합니다.');
    assert((0, hand_precision_region_1.hasUsableHandDetail)({ hasHand: true, handednessScore: 0.7, landmarks, precision: { applied: true, region: { left: 0.35, top: 0.34, right: 0.65, bottom: 0.66 } } }), '정밀 ROI에서 충분히 크게 재검출된 손은 코치 엔진에 전달해야 합니다.');
}
