...
const trajectory_speed_engine_1 = require("../services/trajectory-speed-engine");
function sample(capturedAt, shift = 0) {
...
const coach = new trajectory_speed_engine_1.TrajectorySpeedCoach({
    startBpm: 60,
    targetBpm: 100,
    pulsesPerBeat: 2,
    pattern: 'D U D U',
...
for (let time = 0; time <= 8_000; time += 160) {
    ...
    if (result?.state === 'baseline-ready')
        baselineReady = true;
}
node_assert_1.strict.equal(baselineReady, true, '느린 속도 기준 궤적이 저장되어야 합니다.');
...
node_assert_1.strict.equal(stableSeen, true, '같은 궤적 반복은 안정으로 판정되어야 합니다.');
...
for (let time = 16_100; time <= 24_500; time += 150) {
    ...
    if (result?.state === 'broken') {
        brokenSeen = true;
        node_assert_1.strict.ok(result.reinforcement.length > 10, '붕괴 원인에 맞는 보강훈련이 포함되어야 합니다.');
        node_assert_1.strict.ok(result.lastStableBpm >= 60, '마지막 안정 BPM이 유지되어야 합니다.');
    }
}
node_assert_1.strict.equal(brokenSeen, true, '속도 상승 후 크게 달라진 궤적은 붕괴로 판정되어야 합니다.');
