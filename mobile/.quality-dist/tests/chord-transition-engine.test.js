...
const chord_transition_engine_1 = require("../services/chord-transition-engine");
let checks = 0;
...
function confirmed(chordName, confidencePercent = 90) {
    return {
        status: 'confirmed',
        chordName,
        aliases: [],
...
{
    const target = (0, chord_transition_engine_1.parseChordTransitionTarget)('D→G');
    assert(target?.from === 'D' && target.to === 'G', 'D→G 패턴을 전환 목표로 읽어야 합니다.');
    assert((0, chord_transition_engine_1.parseChordTransitionTarget)('P i m') == null, '아르페지오 패턴을 코드 전환으로 오해하면 안 됩니다.');
}
{
    const tracker = new chord_transition_engine_1.ChordTransitionTracker();
