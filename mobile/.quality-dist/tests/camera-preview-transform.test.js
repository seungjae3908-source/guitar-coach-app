...
const camera_preview_transform_1 = require("../services/camera-preview-transform");
...
}
{
    ...
    const mapped = (0, camera_preview_transform_1.previewPointToImage)(point, preview, image);
    assert(close(mapped.x, point.x) && close(mapped.y, point.y), '같은 화면비 좌표는 변하지 않아야 합니다.');
}
{
    const preview = { width: 360, height: 480 };
    const image = { width: 4032, height: 3024 };
    const metrics = (0, camera_preview_transform_1.cameraCoverMetrics)(preview, image);
    assert(close(metrics.cropX, 140), '4:3 촬영 원본은 3:4 미리보기에서 좌우 140px씩 잘려야 합니다.');
    assert(close(metrics.cropY, 0), '세로 방향 크롭은 없어야 합니다.');
    const left = (0, camera_preview_transform_1.previewPointToImage)({ x: 0, y: 0.5 }, preview, image);
    const right = (0, camera_preview_transform_1.previewPointToImage)({ x: 1, y: 0.5 }, preview, image);
...
}
{
    ...
    for (const point of points) {
        const imagePoint = (0, camera_preview_transform_1.previewPointToImage)(point, preview, image);
        const restored = (0, camera_preview_transform_1.imagePointToPreview)(imagePoint, preview, image);
...
}
{
    const preview = { width: 360, height: 480 };
    const image = { width: 4032, height: 3024 };
    const mapped = (0, camera_preview_transform_1.previewRegionToImage)({ left: 0.2, top: 0.3, right: 0.8, bottom: 0.85 }, preview, image, 0.025);
