...
const technique_analysis_engine_1 = require("../services/technique-analysis-engine");
function assert(condition, message) {
    if (!condition)
        throw new Error(`기술 분석 품질 게이트 실패: ${message}`);
}
function makeSample(index, category, overrides = {}) {
    return {
...
const multiIssues = (0, technique_analysis_engine_1.analyzeTechniqueWindow)(multiIssueSamples);
...
const alternateSamples = Array.from({ length: 10 }, (_, index) => {
    const hit = index < alternateDirections.length
        ? {
...
const alternateIssues = (0, technique_analysis_engine_1.analyzeTechniqueWindow)(alternateSamples);
...
const fingerIssues = (0, technique_analysis_engine_1.analyzeTechniqueWindow)(fingerSamples);
...
const chordIssues = (0, technique_analysis_engine_1.analyzeTechniqueWindow)(chordSamples);
...
const unreliableIssues = (0, technique_analysis_engine_1.analyzeTechniqueWindow)(unreliable);
