...
const power_chord_templates_1 = require("../config/power-chord-templates");
const fretboard_chord_recognizer_1 = require("../services/fretboard-chord-recognizer");
let checks = 0;
...
const fretDistance = (fret) => 1 - 2 ** (-Math.max(0, fret) / 12);
function pointFor(stringNumber, fret) {
...
function observation(finger, stringNumber, fret) {
    return {
        finger,
...
{
    const result = (0, fretboard_chord_recognizer_1.recognizeChord)(cObservations, null, null);
    assert(result.status === 'cannot-judge', '지판 보정 없이 코드 이름이나 점수를 만들면 안 됩니다.');
    assert(result.score == null, '지판 보정 없는 결과는 점수가 없어야 합니다.');
}
{
    const result = (0, fretboard_chord_recognizer_1.recognizeChord)(cObservations, calibration, null);
...
{
    const result = (0, fretboard_chord_recognizer_1.recognizeChord)(cObservations, calibration, cAudio);
...
}
{
    ...
    const result = (0, fretboard_chord_recognizer_1.recognizeChord)(incomplete, calibration, cAudio);
...
{
    const g5Observations = [
        observation('index', 6, 3),
        observation('ring', 5, 5),
        observation('pinky', 4, 5),
    ];
    const result = (0, fretboard_chord_recognizer_1.recognizeChord)(g5Observations, calibration, {
        pitchClasses: [7, 2],
        confidence: 0.94,
        signalToNoiseDb: 25,
        clippingRatio: 0.001,
    }, power_chord_templates_1.POWER_CHORD_TEMPLATES);
...
{
    const tracker = new fretboard_chord_recognizer_1.ChordRecognitionTracker();
    const candidate = (0, fretboard_chord_recognizer_1.recognizeChord)(cObservations, calibration, null);
