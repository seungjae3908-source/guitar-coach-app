...
exports.AutomaticGuitarGate = void 0;
exports.isPlausiblePlayingHand = isPlausiblePlayingHand;
exports.evaluateAutomaticGuitarDetection = evaluateAutomaticGuitarDetection;
...
function pointInRegion(point, region, margin = 0) {
...
function isPlausiblePlayingHand(hand) {
    if (!hand.hasHand || hand.landmarks.length < 21 || hand.handednessScore < 0.25)
        return false;
    ...
    if (palm < 0.035 || palm > 0.30 || width < 0.055 || width > 0.62 || height < 0.055 || height > 0.72)
        return false;
    const fingerPairs = [[4, 2], [8, 5], [12, 9], [16, 13], [20, 17]];
...
function averageStringSpacing(lines) {
    if (lines.length < 2)
        return 0;
    const lineLength = (line) => Math.hypot(line.endX - line.startX, line.endY - line.startY);
...
function playingPoint(hand) {
    const indices = [4, 8, 12, 16];
    const points = indices.map((index) => hand.landmarks[index]).filter(finitePoint);
    if (!points.length)
        return regionCenter(handBounds(hand.landmarks.slice(0, 21)));
    return {
        x: points.reduce((sum, point) => sum + point.x, 0) / points.length,
        y: points.reduce((sum, point) => sum + point.y, 0) / points.length,
    };
}
function structureResonator(structure) {
...
function expandPlayingRegion(hand, resonator, bridge, body) {
    ...
    const ensure = (start, end, center, minimum, maximum) => {
        let width = end - start;
        if (width < minimum) {
...
function evaluateAutomaticGuitarDetection(hand, strings, structure) {
    ...
    const empty = (reason, confidence = 0, nearbyFingerCount = 0) => ({
        accepted: false,
    ...
    if (!isPlausiblePlayingHand(hand))
        return empty('기타 위의 정상적인 손 21개 관절을 찾지 못했습니다.', strings.confidence * 0.42);
    const spacing = averageStringSpacing(strings.lines);
    if (spacing <= 0.002 || spacing >= 0.15)
        return empty('기타줄 간격이 비정상입니다.', strings.confidence * 0.55);
    ...
    const bodyDiagonal = Math.hypot(structure.bodyBox.right - structure.bodyBox.left, structure.bodyBox.bottom - structure.bodyBox.top);
    ...
    const geometryScore = clamp(structure.objectConfidence * 0.24
        + structure.bodyConfidence * 0.16
        + structure.neckConfidence * 0.15
        + resonator.confidence * 0.15
        + structure.bridgeConfidence * 0.15
        + structure.structureConfidence * 0.15, 0, 1);
    const confidence = clamp(geometryScore * 0.48
        + strings.confidence * 0.22
        + hand.handednessScore * 0.14
        + overlapScore * 0.10
        + (1 - clamp(playingZoneDistance / 0.18, 0, 1)) * 0.06, 0, 1);
    if (confidence < 0.50) {
...
function averageRegion(regions) {
...
class AutomaticGuitarGate {
    required;
    maximumCenterDrift;
    maximumAngleDifference;
    maximumAnchorDrift;
    detections = [];
    constructor(required = 5, maximumCenterDrift = 0.09, maximumAngleDifference = 11, maximumAnchorDrift = 0.075) {
        this.required = required;
        this.maximumCenterDrift = maximumCenterDrift;
        this.maximumAngleDifference = maximumAngleDifference;
        this.maximumAnchorDrift = maximumAnchorDrift;
    }
    reset() {
        this.detections = [];
    }
    add(detection) {
        if (!detection.accepted || !detection.region) {
            this.reset();
            return { locked: false, consecutive: 0, required: this.required, region: null, confidence: 0 };
        }
        ...
        if (previous?.region) {
            ...
            const objectDrift = distance({ x: previous.objectCenterX, y: previous.objectCenterY }, { x: detection.objectCenterX, y: detection.objectCenterY });
            const resonatorDrift = distance({ x: previous.resonatorCenterX, y: previous.resonatorCenterY }, { x: detection.resonatorCenterX, y: detection.resonatorCenterY });
            const bridgeDrift = distance({ x: previous.bridgeCenterX, y: previous.bridgeCenterY }, { x: detection.bridgeCenterX, y: detection.bridgeCenterY });
            const rotation = angleDifference(previous.angleDegrees, detection.angleDegrees);
            if (centerDrift > this.maximumCenterDrift
        ...
        if (this.detections.length > this.required)
...
exports.AutomaticGuitarGate = AutomaticGuitarGate;
