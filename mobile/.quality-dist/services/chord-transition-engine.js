...
exports.ChordTransitionTracker = void 0;
exports.parseChordTransitionTarget = parseChordTransitionTarget;
function normalizeChord(value) {
    return value.trim().replace(/♯/g, '#').replace(/♭/g, 'b');
}
function parseChordTransitionTarget(pattern) {
...
class ChordTransitionTracker {
    ...
    process(recognition, pattern, capturedAt = Date.now()) {
        const target = parseChordTransitionTarget(pattern);
        if (!target || recognition.status !== 'confirmed' || !recognition.chordName)
            return null;
...
exports.ChordTransitionTracker = ChordTransitionTracker;
