...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`연속 인식 품질 게이트 실패: ${message}`);
}
...
function landmarks(offsetX = 0, offsetY = 0): HandLandmarkPoint[] {
...
function lines(customY?: number[]): GuitarStringLine[] {
  const ys = customY ?? [0.30, 0.35, 0.40, 0.45, 0.50, 0.55];
  return ys.map((y, index) => ({
    visualIndex: (index + 1) as 1 | 2 | 3 | 4 | 5 | 6,
    stringNumber: (6 - index) as 1 | 2 | 3 | 4 | 5 | 6,
    startX: 0.02,
    startY: y,
    endX: 0.98,
    endY: y + 0.002,
...
function tracking(overrides: Partial<GuitarStringTrackingResult> = {}): GuitarStringTrackingResult {
  return {
    detected: true,
    confidence: 0.86,
    angleDegrees: 0.1,
...
function hit(capturedAt: number): QualityStringHit {
  return {
    capturedAt,
    contactId: 'index',
    label: 'i',
...
const gate = new ContinuousTrackingQualityGate();
...
const geometryGate = new ContinuousTrackingQualityGate();
