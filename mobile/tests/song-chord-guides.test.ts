...
let checks = 0;
function assert(condition: unknown, message: string) { checks += 1; if (!condition) throw new Error(message); }
...
assert(chordAtSectionProgress(guide.chords, 0, guide.beatWeights).current === 'G', '구간 시작 코드를 반환해야 합니다.');
assert(chordAtSectionProgress(guide.chords, 0.3, guide.beatWeights).current === 'Em', '진행률과 박 길이에 맞는 현재 코드를 반환해야 합니다.');
assert(chordAtSectionProgress(guide.chords, 0.99, guide.beatWeights).next === 'G', '마지막 코드 다음은 반복 첫 코드여야 합니다.');
