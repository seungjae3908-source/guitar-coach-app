...
import {
  analyzeRightHandStringRoles,
} from '../services/right-hand-string-role-engine';
import type {
  RightHandFingerId,
...
function hit(capturedAt: number, contactId: string, stringNumber: number, direction = 'up') {
  return {
    capturedAt,
    contactId,
...

{
  ...
  const feedback = analyzeRightHandStringRoles(samples);
...
{
  const samples: RightHandTechniqueSample[] = [];
  for (let stroke = 0; stroke < 10; stroke += 1) {
    ...
    samples.push(baseSample(
      capturedAt,
      'strumming',
      strings.map((stringNumber, index) => hit(capturedAt + index * 12, 'pick', stringNumber, down ? 'down' : 'up')),
    ));
  }
  const feedback = analyzeRightHandStringRoles(samples);
...

{
  ...
  while (samples.length < 10) samples.push(baseSample(9_500 + samples.length * 100, 'downPicking', []));
  const feedback = analyzeRightHandStringRoles(samples);
