...
assert.equal(pointInsideRegion({ x: 0.31, y: 0.34 }, region), false, '영상의 목·가슴 오검출 위치는 ROI 밖이어야 합니다.');
assert.equal(pointInsideRegion({ x: 0.53, y: 0.70 }, region), true, '사운드홀 위 실제 오른손 위치는 ROI 안이어야 합니다.');
...
const validHand = validateHandInRegion(validLandmarks, region);
...
const falseBody = validateHandInRegion(falseBodyLandmarks, region);
assert.equal(falseBody.valid, false, '목·가슴에 생긴 가짜 관절은 승인되면 안 됩니다.');

const gate = new ConsecutiveHandGate(5, 0.17);
