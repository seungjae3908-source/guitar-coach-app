...
function projected(
  finger: FingeringProjectedPosition['finger'],
  stringNumber: FingeringProjectedPosition['stringNumber'],
  fret: number,
  motion = 0.03,
): FingeringProjectedPosition {
  return {
    finger,
    stringNumber,
    fret,
...
function note(
  capturedAt: number,
  finger: FingeringNoteEvent['finger'],
  fret: number,
  stringNumber: FingeringNoteEvent['stringNumber'] = 6,
): FingeringNoteEvent {
  return {
    capturedAt,
    status: 'confirmed',
...
{
  const target = parseFingeringTarget('1-2-3-4', 'fingering');
...
{
  const target = parseFingeringTarget('S6-5-i S6-7-r S6-8-k S5-5-i S5-7-r S5-8-k', 'scales');
...

{
  ...
  const events = Array.from({ length: 8 }, (_, index) => note(
    2_000 + index * 180,
    fingers[index % 4],
    5 + index % 4,
  ));
...

{
  const result = analyzeFingeringEvents({
    events: [],
    target: parseFingeringTarget('1-2-3-4', 'fingering'),
