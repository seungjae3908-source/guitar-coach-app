...
import LiveCoachCameraOnly from './LiveCoachCameraOnly';
import {
...
const SUBDIVISION_OPTIONS: SubdivisionOption[] = [
  ...
  { value: 3, label: '셋잇단', countExample: '원 트립 렛 · 투 트립 렛' },
  { value: 4, label: '16분', countExample: '원 이 앤 어 · 투 이 앤 어' },
];
...
export default function LiveCoachTestAppV057() {
  ...
  useEffect(() => {
    ...
    void updateAdvancedMetronomeAsync(
      ...
      setError(caught instanceof Error ? caught.message : '변경한 BPM을 적용하지 못했습니다.');
      setRunning(false);
    });
  ...
  useEffect(() => {
    ...
    visualTimerRef.current = setTimeout(tick, 60);
  ...
  const toggleRunning = async () => {
    ...
    try {
      ...
      if (!isAdvancedMetronomeAvailable) {
        throw new Error('이 설치본에는 고급 메트로놈 모듈이 없습니다.');
      }
      if (!soundEnabled && !voiceEnabled) {
        throw new Error('클릭음 또는 사람 음성 중 하나를 켜 주세요.');
      }
    ...
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        <View style={styles.headerTextWrap}>
          <Text style={styles.headerTitle}>고급 메트로놈 0.5.7</Text>
          <Text style={styles.headerStatus}>
            {running ? `${meter.label} · 엔진 ${bpm} BPM · ${currentToken}` : '연속 클릭·사람 음성 카운트'}
          </Text>
        </View>
        <Pressable
          accessibilityRole="button"
      ...
      {expanded ? (
        <ScrollView style={styles.panelScroll} contentContainerStyle={styles.panel} nestedScrollEnabled>
          ...
          <Text style={styles.sectionTitle}>BPM · 실행 중에도 즉시 변경</Text>
          ...
          <View style={styles.switchRow}>
            <View style={styles.switchTextWrap}>
              ...
              <Text style={styles.switchDetail}>선택한 분할마다 클릭하고 첫 박을 강조합니다.</Text>
            </View>
          ...
          <View style={styles.actionRow}>
            ...
            >
              <Text style={styles.secondaryButtonText}>사람 음성 테스트</Text>
            </Pressable>
          </View>

          {voiceEnabled && subdivision >= 3 ? (
            <Text style={styles.noticeText}>사람 음성은 한 박마다 “원 이 앤 어”처럼 읽고, 클릭은 각 세부 박마다 납니다.</Text>
          ) : null}
          {error ? <Text style={styles.errorText}>{error}</Text> : null}
          <Text style={styles.compatText}>아래에는 카메라 자세 분석만 있으며 중복 메트로놈은 제거했습니다.</Text>
        </ScrollView>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  headerStatus: { color: '#7ee787', fontSize: 11, fontWeight: '800', marginTop: 3 },
  ...
  panelScroll: { maxHeight: 400, backgroundColor: '#0d1117', borderBottomWidth: 1, borderBottomColor: '#30363d' },
  ...
  primaryButton: { flex: 1, minHeight: 47, borderRadius: 13, backgroundColor: '#2ea043', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10 },
  stopButton: { backgroundColor: '#da3633' },
  primaryButtonText: { color: '#ffffff', fontSize: 12, fontWeight: '900', textAlign: 'center' },
  secondaryButton: { flex: 1, minHeight: 47, borderRadius: 13, borderWidth: 1, borderColor: '#30363d', backgroundColor: '#21262d', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10 },
  secondaryButtonText: { color: '#f0f6fc', fontSize: 12, fontWeight: '900', textAlign: 'center' },
  noticeText: { color: '#f2cc60', fontSize: 10, lineHeight: 16, marginTop: 10 },
