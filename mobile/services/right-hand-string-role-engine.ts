...
} from './right-hand-technique-engine';
...
function resolvedHits(samples: RightHandTechniqueSample[]) {
  const deduped = new Map<string, RightHandHitSample>();
  samples.flatMap((sample) => sample.hits).forEach((hit) => {
    if (hit.confidence < 0.55 || hit.stringNumber < 1 || hit.stringNumber > 6) return;
    const key = `${hit.capturedAt}-${hit.contactId}-${hit.stringNumber}-${hit.direction}`;
    const previous = deduped.get(key);
    if (!previous || hit.confidence > previous.confidence) deduped.set(key, hit);
  });
  return [...deduped.values()].sort((left, right) => left.capturedAt - right.capturedAt);
}

function targetLabel(finger: RightHandFingerId) {
  if (finger === 'thumb') return '6·5·4번 베이스줄';
  if (finger === 'index') return '3번 줄';
  if (finger === 'middle') return '2번 줄';
  if (finger === 'ring') return '1번 줄';
  return '지정 줄';
}

function correctString(finger: RightHandFingerId, stringNumber: number) {
  if (finger === 'thumb') return stringNumber >= 4 && stringNumber <= 6;
  if (finger === 'index') return stringNumber === 3;
  if (finger === 'middle') return stringNumber === 2;
  if (finger === 'ring') return stringNumber === 1;
  return true;
}

function fingerLabel(finger: RightHandFingerId) {
  if (finger === 'thumb') return '엄지 P';
  if (finger === 'index') return '검지 i';
  if (finger === 'middle') return '중지 m';
  if (finger === 'ring') return '약지 a';
  return '새끼손가락';
}
...
function parseExpectedFingers(pattern: string | undefined): RightHandFingerId[] {
  ...
  (pattern?.match(/[Ppima]/g) ?? []).forEach((token) => {
    ...
    const finger: RightHandFingerId = normalized === 'p'
      ? 'thumb'
      : normalized === 'i'
        ? 'index'
        : normalized === 'm'
          ? 'middle'
          : 'ring';
    if (!ids.includes(finger)) ids.push(finger);
  });
  return ids.length ? ids : ['thumb', 'index', 'middle', 'ring'];
}

function analyzeArpeggioStrings(
  samples: RightHandTechniqueSample[],
  hits: RightHandHitSample[],
  feedback: RightHandTechniqueFeedback[],
) {
  const confidence = confidencePercent(samples);
  const expectedFingers = parseExpectedFingers(samples.at(-1)?.pattern);
  const fingerHits = hits.filter((hit) => expectedFingers.includes(hit.contactId as RightHandFingerId));
  if (fingerHits.length < 6) return;
...
function groupStrokes(hits: RightHandHitSample[]) {
  ...
  hits.forEach((hit) => {
    ...
    if (previous && previous.direction === hit.direction && hit.capturedAt - previous.capturedAt <= 150) {
      if (!previous.strings.includes(hit.stringNumber)) previous.strings.push(hit.stringNumber);
...
function analyzeStrumRoles(
  ...
  if (strokes.length < 6) return;

  let repeatedDirection = 0;
  for (let index = 1; index < strokes.length; index += 1) {
    if (strokes[index].direction === strokes[index - 1].direction) repeatedDirection += 1;
  }
  ...
  if (latest.category === 'alternatePicking') {
    ...
    if (stringChanges.length >= 3) {
      ...
      for (let index = 1; index < pickHits.length; index += 1) {
        if (pickHits[index].stringNumber === pickHits[index - 1].stringNumber) continue;
        if (pickHits[index].direction === pickHits[index - 1].direction) brokenChanges += 1;
      }
...
export function analyzeRightHandStringRoles(samples: RightHandTechniqueSample[]): RightHandTechniqueFeedback[] {
  const recent = recentWindow(samples);
  const latest = recent.at(-1);
  if (!latest || recent.length < 10) return [];
  if (latest.stringConfidence < 0.52 || latest.stringStability < 0.5 || latest.visibleStringCount < 5) return [];
  ...
  return feedback
    .sort((left, right) => {
      const rank = (status: RightHandTechniqueFeedback['status']) => status === 'warning' ? 4 : status === 'correction' ? 3 : status === 'cannot-judge' ? 2 : 1;
