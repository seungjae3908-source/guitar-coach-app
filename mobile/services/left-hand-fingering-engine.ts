...
export type FingeringProjectedPosition = FrettedPosition & {
  motion?: number;
};

export type FingeringAudioAttack = {
  capturedAt: number;
  frequencyHz: number;
...
export type FingeringNoteEvent = {
  capturedAt: number;
...
export type FingeringTargetStep = {
  finger: FrettingFingerId;
  stringNumber?: GuitarStringNumber;
  fret?: number;
  relativeFret?: number;
...
export type FingeringTarget = {
  id: string;
  label: string;
  kind: 'relative-chromatic' | 'absolute-sequence';
  steps: FingeringTargetStep[];
};

export type FingeringAnalysisResult = {
  status: 'cannot-judge' | 'candidate' | 'confirmed';
  score: number | null;
  confidencePercent: number;
  targetLabel: string;
  events: FingeringNoteEvent[];
  latestEvent: FingeringNoteEvent | null;
  corrections: string[];
  positives: string[];
  evidence: string[];
  fingerAccuracyPercent: number;
  positionAccuracyPercent: number;
  timingStabilityPercent: number;
};

const OPEN_STRING_MIDI: Record<GuitarStringNumber, number> = {
  6: 40,
  5: 45,
  4: 50,
  3: 55,
...
const FINGER_FROM_NUMBER: Record<string, FrettingFingerId> = {
  '1': 'index',
  '2': 'middle',
  '3': 'ring',
  '4': 'pinky',
...
const FINGER_FROM_TOKEN: Record<string, FrettingFingerId> = {
  i: 'index',
  m: 'middle',
  r: 'ring',
  k: 'pinky',
...
const FINGER_KO: Record<FrettingFingerId, string> = {
  index: '검지',
  middle: '중지',
  ring: '약지',
  pinky: '새끼손가락',
...
export function parseFingeringTarget(
  pattern: string | undefined,
  category: PracticeCategoryId,
): FingeringTarget | null {
  const normalized = normalizePattern(pattern);
  const numberSequence = normalized.match(/[1-4]/g) ?? [];
  if (
    normalized.includes('1-2-3-4')
  ...
  const absoluteSteps: FingeringTargetStep[] = [];
  const expression = /(?:s)?([1-6])[-:f]([0-9]{1,2})[-:(]?([imrkp])\)?/g;
  let match = expression.exec(normalized);
  while (match) {
    const stringNumber = Number(match[1]) as GuitarStringNumber;
...
function expectedMidi(position: FingeringProjectedPosition) {
  return OPEN_STRING_MIDI[position.stringNumber] + position.fret;
}

export function matchFingeringAttack(
  attack: FingeringAudioAttack,
  positions: FingeringProjectedPosition[],
): FingeringNoteEvent | null {
...
function expectedForEvent(
  ...
  if (target.kind === 'absolute-sequence') return step;
...
function timingStability(events: FingeringNoteEvent[]) {
...
function mostFrequentMismatch(
  mismatches: Array<{
    event: FingeringNoteEvent;
    expected: FingeringTargetStep;
    fingerWrong: boolean;
...
export function analyzeFingeringEvents(options: {
  events: FingeringNoteEvent[];
  target: FingeringTarget | null;
  calibrationAvailable: boolean;
  microphoneEnabled: boolean;
}): FingeringAnalysisResult {
  const events = options.events.slice(-24);
  const targetLabel = options.target?.label ?? '줄·프렛 순서';
  const cannot = (correction: string, evidence: string): FingeringAnalysisResult => ({
    status: 'cannot-judge',
    score: null,
    confidencePercent: 0,
    targetLabel,
  ...
  if (mismatch) corrections.push(mismatch);
  ...
  if (fingerAccuracyPercent >= 88) positives.push('손가락 순서가 목표 패턴과 안정적으로 맞습니다.');
  if (positionAccuracyPercent >= 88) positives.push('실제로 울린 줄·프렛이 목표 위치와 정확히 맞습니다.');
  if (expectedPinky && pinkyCount >= 2) positives.push('새끼손가락까지 빠뜨리지 않고 사용하고 있습니다.');
  if (timingStabilityPercent >= 78) positives.push('각 음의 시간 간격이 일정하게 유지됩니다.');
