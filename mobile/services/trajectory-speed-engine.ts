...
export type TrajectoryCoachState =
  | 'waiting'
  | 'collecting-baseline'
  | 'baseline-ready'
  | 'stable'
...
export type TrajectoryCoachResult = {
  capturedAt: number;
  state: TrajectoryCoachState;
  title: string;
  observation: string;
  cause: string;
  correction: string;
  ...
  currentBpm: number;
  baselineBpm: number;
  lastStableBpm: number;
  stableCycles: number;
  brokenCycles: number;
  shouldIncreaseBpm: boolean;
  shouldReturnToStableBpm: boolean;
};

type Vector = number[];
type BaselineBin = { sum: Vector; count: number };

type TrajectoryCoachOptions = {
  startBpm: number;
  targetBpm: number;
...
function patternSteps(pattern?: string) {
  const tokens = pattern?.match(/[PpIiMmAaDdUu]/g) ?? [];
  return clamp(tokens.length || 4, 2, 12);
}

function emptyVector(length: number) {
  return Array.from({ length }, () => 0);
}

function addVectors(left: Vector, right: Vector) {
  return left.map((value, index) => value + (right[index] ?? 0));
}

function divideVector(vector: Vector, divisor: number) {
...
function guidance(cause: string, deviationPercent: number, bpm: number, lastStableBpm: number) {
...
export class TrajectorySpeedCoach {
  private readonly startBpm: number;
  private readonly targetBpm: number;
  private readonly pulsesPerBeat: number;
  private readonly cycleSteps: number;
  private readonly baselineBins: BaselineBin[];
  private baselineSamples: Array<{ bin: number; vector: Vector }> = [];
  private baselineStartedAt = 0;
  private baselineReady = false;
  private currentBpm: number;
  private lastStableBpm: number;
  private phaseStartedAt = 0;
  private cycleIndex = -1;
  private cycleDistances: number[] = [];
  private cycleCauses: string[] = [];
  private stableCycles = 0;
  private brokenCycles = 0;
  private lastResult: TrajectoryCoachResult;

  constructor(options: TrajectoryCoachOptions) {
  ...
  start(capturedAt = Date.now()) {
    ...
    this.lastResult = this.result(
      ...
      capturedAt,
      '느린 속도 기준 궤적을 만드는 중',
      '힘을 빼고 올바른 자세와 궤적으로 같은 패턴을 반복하세요.',
    );
    return this.lastResult;
  }

  updateBpm(nextBpm: number, capturedAt = Date.now()) {
  ...
  addSample(sample: MotionSample): TrajectoryCoachResult | null {
    if (sample.handConfidence < 0.48 || sample.wristConfidence < 0.38 || sample.palmSize < 0.06) {
      this.lastResult = this.result(
        'cannot-judge',
        sample.capturedAt,
        '궤적 정밀 판정 불가',
        `손 ${Math.round(sample.handConfidence * 100)}% · 손목 ${Math.round(sample.wristConfidence * 100)}%로 기준 궤적을 비교하기 부족합니다.`,
      );
      return this.lastResult;
    }
    ...
    if (!this.baselineReady) {
      ...
      if (this.canFinishBaseline(sample.capturedAt)) {
        ...
        if (noise <= 0.30) {
          ...
          this.lastResult = this.result(
            'baseline-ready',
            sample.capturedAt,
            `${this.startBpm} BPM 기준 궤적 저장 완료`,
            `느린 기준의 반복 편차가 ${Math.round(noise * 100)}%입니다. 이제 속도를 올려도 같은 궤적이 유지되는지 비교합니다.`,
          );
          return this.lastResult;
        }
        if (sample.capturedAt - this.baselineStartedAt >= 12_000) {
          this.lastResult = this.result(
            ...
            sample.capturedAt,
            '기준 궤적이 아직 불안정합니다',
            `느린 속도 반복 편차가 ${Math.round(noise * 100)}%입니다. 속도를 올리지 말고 같은 동작을 더 작게 반복하세요.`,
          );
          return this.lastResult;
        }
      }
      this.lastResult = this.result(
        ...
        sample.capturedAt,
        '느린 속도 기준 궤적 수집 중',
        `${this.baselineSamples.length}/${BASELINE_MIN_SAMPLES}개 표본 · 같은 자세로 계속 반복하세요.`,
      );
      return null;
    }
    ...
    if (averageDeviation <= STABLE_DEVIATION) {
      this.stableCycles += 1;
      this.brokenCycles = 0;
      if (this.stableCycles >= 2) this.lastStableBpm = Math.max(this.lastStableBpm, this.currentBpm);
      this.lastResult = this.result(
        'stable',
        sample.capturedAt,
        `${this.currentBpm} BPM에서도 기준 궤적 유지`,
        `느린 기준 대비 궤적 편차 ${deviationPercent}% · ${this.stableCycles}회 연속 안정입니다.`,
        deviationPercent,
      );
      return this.lastResult;
    }
    ...
    this.lastResult = this.result(
      ...
      sample.capturedAt,
      '기준 궤적 경계 구간',
      `현재 편차 ${deviationPercent}%입니다. 속도를 유지하고 같은 동작을 2회 더 반복하세요.`,
      deviationPercent,
    );
    return this.lastResult;
  }
  ...
  private baselineVector(binIndex: number) {
    const exact = this.baselineBins[binIndex];
    if (exact?.count) return divideVector(exact.sum, exact.count);
    for (let offset = 1; offset < BIN_COUNT; offset += 1) {
      ...
      if (left.count) return divideVector(left.sum, left.count);
      if (right.count) return divideVector(right.sum, right.count);
    }
    return null;
  }

  private canFinishBaseline(capturedAt: number) {
