...
export type RightHandFingerId = 'thumb' | 'index' | 'middle' | 'ring' | 'pinky';
export type RightHandPoint = { x: number; y: number };

export type RightHandFingerSample = {
  tip: RightHandPoint;
  base: RightHandPoint;
  pip: RightHandPoint;
  jointAngle: number;
...
export type RightHandContactSample = {
  id: string;
  visualIndex: number;
  stringNumber: number;
...
export type RightHandHitSample = {
  capturedAt: number;
  contactId: string;
  visualIndex: number;
  stringNumber: number;
...
export type RightHandTechniqueSample = {
  capturedAt: number;
  ...
  pick: {
    ...
    angleDegrees: number;
    exposure: number;
    center: RightHandPoint;
  };
  stringAngle: number | null;
  stringConfidence: number;
  stringStability: number;
  visibleStringCount: number;
  fingers: Record<RightHandFingerId, RightHandFingerSample>;
  contacts: RightHandContactSample[];
  hits: RightHandHitSample[];
};

export type RightHandTechniqueFeedback = {
  id: string;
  status: 'cannot-judge' | 'correction' | 'warning' | 'success';
  title: string;
  instruction: string;
  evidence: string;
  nextGoal: string;
...
const RIGHT_HAND_CATEGORIES = new Set<PracticeCategoryId>([
...
function distance(left: RightHandPoint, right: RightHandPoint) {
...
function correlation(left: number[], right: number[]) {
  if (left.length !== right.length || left.length < 6) return 0;
...
function recentWindow(samples: RightHandTechniqueSample[]) {
  const latest = samples.at(-1);
  if (!latest || !RIGHT_HAND_CATEGORIES.has(latest.category)) return [];
  return samples
...
function motionSteps(samples: RightHandTechniqueSample[]): MotionStep[] {
...
function usableHits(samples: RightHandTechniqueSample[]) {
  return samples.flatMap((sample) => sample.hits)
    .filter((hit) => hit.confidence >= 0.50)
    .sort((left, right) => left.capturedAt - right.capturedAt);
}

function groupStrokes(hits: RightHandHitSample[]) {
  const groups: Stroke[] = [];
  hits.forEach((hit) => {
    if (hit.direction !== 'down' && hit.direction !== 'up') return;
    ...
    if (previous && previous.direction === hit.direction && hit.capturedAt - previous.capturedAt <= 145) {
      if (!previous.strings.includes(string)) previous.strings.push(string);
...
function parseExpectedPattern(pattern: string | undefined) {
  const tokens = pattern?.match(/[Ppima]/g) ?? [];
  return tokens.map((token): RightHandFingerId => {
    const normalized = token.toLowerCase();
    if (normalized === 'p') return 'thumb';
    if (normalized === 'i') return 'index';
    if (normalized === 'm') return 'middle';
    return 'ring';
  });
}

function sequenceMismatchRatio(actual: RightHandFingerId[], expected: RightHandFingerId[]) {
  if (!expected.length || actual.length < expected.length) return null;
  let best = 1;
  for (let offset = 0; offset < expected.length; offset += 1) {
    let mismatches = 0;
    actual.forEach((finger, index) => {
      if (finger !== expected[(index + offset) % expected.length]) mismatches += 1;
    });
...
function returnDelays(samples: RightHandTechniqueSample[], finger: RightHandFingerId, hits: RightHandHitSample[]) {
  const delays: number[] = [];
  hits.filter((hit) => hit.contactId === finger).forEach((hit) => {
    ...
    const returned = samples.slice(hitIndex + 1).find((sample) => (
      sample.capturedAt - hit.capturedAt <= 520
...
function analyzePickReadiness(samples: RightHandTechniqueSample[], feedback: RightHandTechniqueFeedback[]) {
  const latest = samples.at(-1)!;
...
function analyzeStrumming(samples: RightHandTechniqueSample[], feedback: RightHandTechniqueFeedback[]) {
  const confidence = confidencePercent(samples);
  ...
  const wristTravel = Math.hypot(
    range(samples.map((sample) => sample.wrist.x)),
    range(samples.map((sample) => sample.wrist.y)),
  ) / palm;
  ...
  if (alignment < 0.64) {
    ...
    feedback.push(success(
      'strum-path-good',
      '스트럼 방향이 줄 수직축을 안정적으로 통과합니다',
      `궤도 일치도가 ${Math.round(alignment * 100)}%입니다.`,
      '같은 축을 유지하며 속도만 조금씩 올리세요.',
      confidence,
      [{ label: '궤도 일치', value: `${Math.round(alignment * 100)}%` }],
    ));
  }
  ...
  if (wristTravel > 0.78 && palmAngleRange < 9 && (speedVariation > 0.48 || jerkRatio > 0.42)) {
    ...
    feedback.push(success(
      'strum-relaxation-good',
      '손목 회전과 스트럼 속도가 자연스럽게 이어집니다',
      `손바닥 각도 변화 ${Math.round(palmAngleRange)}°, 속도 흔들림 ${Math.round(speedVariation * 100)}%입니다.`,
      '힘을 더 주지 말고 같은 느낌으로 8회 유지하세요.',
      confidence,
      [
        { label: '손목 회전', value: `${Math.round(palmAngleRange)}°` },
        { label: '속도 흔들림', value: `${Math.round(speedVariation * 100)}%` },
...
function analyzePicking(samples: RightHandTechniqueSample[], feedback: RightHandTechniqueFeedback[]) {
  const confidence = confidencePercent(samples);
  ...
  if (samples.at(-1)?.category === 'alternatePicking' && directions.length >= 6) {
    ...
    for (let index = 1; index < directions.length; index += 1) {
      if (directions[index] === directions[index - 1]) repeats += 1;
    }
    ...
    if (repeatRatio > 0.22) {
      ...
      feedback.push(success(
        'alternate-direction-good',
        '다운·업 교대가 안정적으로 유지됩니다',
        `같은 방향 반복이 ${Math.round(repeatRatio * 100)}%입니다.`,
        '현재 이동 폭을 유지한 채 BPM을 조금씩 올리세요.',
        confidence,
        [{ label: '방향 반복', value: `${Math.round(repeatRatio * 100)}%` }],
      ));
    }
  }
...
function analyzeArpeggio(samples: RightHandTechniqueSample[], feedback: RightHandTechniqueFeedback[]) {
  const confidence = confidencePercent(samples);
  const hits = usableHits(samples).filter((hit) => ['thumb', 'index', 'middle', 'ring'].includes(hit.contactId));
  const actual = hits.map((hit) => hit.contactId as RightHandFingerId);
  ...
  if (expected.length && mismatch != null && mismatch > 0.28) {
    ...
    feedback.push(success(
      'arpeggio-pattern-good',
      '아르페지오 손가락 순서가 목표 패턴과 맞습니다',
      `순서 불일치가 ${Math.round(mismatch * 100)}%입니다.`,
      '순서를 유지하며 음 사이 간격을 일정하게 만드세요.',
      confidence,
      [{ label: '순서 불일치', value: `${Math.round(mismatch * 100)}%` }],
    ));
  }

  (['thumb', 'index', 'middle', 'ring'] as RightHandFingerId[]).forEach((finger) => {
...
export function analyzeRightHandTechniqueWindow(samples: RightHandTechniqueSample[]): RightHandTechniqueFeedback[] {
  ...
  const feedback: RightHandTechniqueFeedback[] = [];
  if (PICK_CATEGORIES.has(latest.category)) analyzePickReadiness(recent, feedback);
  if (latest.category === 'strumming') analyzeStrumming(recent, feedback);
  else if (PICK_CATEGORIES.has(latest.category)) analyzePicking(recent, feedback);
  if (FINGER_CATEGORIES.has(latest.category)) analyzeArpeggio(recent, feedback);

  return feedback
    .sort((left, right) => {
      const statusRank = (status: RightHandTechniqueFeedback['status']) => status === 'warning' ? 4 : status === 'correction' ? 3 : status === 'cannot-judge' ? 2 : 1;
