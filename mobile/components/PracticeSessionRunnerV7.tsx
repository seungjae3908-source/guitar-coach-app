...
function feedbackTone(result: TrajectoryCoachResult | null) {
  if (!result) return styles.feedbackNeutral;
  ...
  return styles.feedbackNeutral;
}

export default function PracticeSessionRunnerV7({
  mode,
  voiceCoachEnabled,
  onClose,
}: {
  ...
  const cameraHeight = focusV7CameraHeight(width, height);
  ...
  useEffect(() => {
    ...
    void loadFocusV7RightHandRegion(cameraFacing).then((stored) => {
      if (cancelled) return;
  ...
  const stopLesson = async (closeAfter = false) => {
    ...
    if (startedAt) {
      ...
      const record: PracticeSessionRecord = {
        id: `${startedAt}-${preset.id}-focus-v7`,
        guitarMode: mode,
        category: preset.category,
        presetId: preset.id,
        title: preset.title,
        ...
        nextAssignment: final?.reinforcement || '오른손 잠금이 유지되는 위치에서 느린 기준 궤적을 다시 수집하세요.',
        cameraMode: preset.cameraFocus,
        microphoneUsed: false,
        metronomeUsed: isAdvancedMetronomeAvailable,
        notes: `FOCUS V7 · 캡처 ${capturedFrames} · 승인 ${acceptedFrames} · 음성 설정 ${voiceCoachEnabled ? '켜짐' : '꺼짐'} · 마이크 완전 미사용`,
      };
      try {
  ...
  const coachingAllowed = canShowFocusV7Coaching(evidence);
  const waitingMessage = focusV7WaitingMessage(evidence);
      ...
      <SafeAreaView style={styles.root}>
        ...
        {!calibrationChecked ? (
          <View style={styles.loadingSurface}>
            <Text style={styles.loadingBuild}>FOCUS V7 · v20</Text>
          ...
          <View style={styles.practiceRoot}>
            <View style={styles.header}>
              <View style={styles.headerCopy}>
                <Text style={styles.buildBadge}>FOCUS V7 · v20 · 마이크 OFF</Text>
            ...
            <View style={styles.routineRow}>
              <View style={styles.routineCopy}>
                <Text style={styles.routineLabel}>현재 루틴</Text>
                <Text style={styles.routineTitle} numberOfLines={1}>{preset.title}</Text>
              </View>
              {matchingPresets.length > 1 ? (
                <Pressable disabled={running} onPress={cycleRoutine} style={[styles.routineButton, running && styles.disabled]}>
                  <Text style={styles.routineButtonText}>다음 루틴</Text>
                </Pressable>
              ) : null}
            </View>
            ...
            <View style={styles.primaryControlRow}>
              ...
              >
                ...
                <Text style={styles.startButtonSub}>{running ? '메트로놈·궤적 비교 실행 중' : '관절 추적은 이미 실행 중 · 마이크는 사용하지 않음'}</Text>
              </Pressable>
            ...
            <View style={[styles.cameraFrame, { height: cameraHeight }]}>
              <FocusCoachCameraV7
                ...
                onHandLockChange={setHandLocked}
              />
            </View>

            <View style={styles.recognitionStrip}>
              <View style={[styles.statusDot, handLocked && styles.statusDotReady]} />
              <View style={styles.recognitionCopy}>
                <Text style={styles.recognitionTitle}>{handLocked ? '오른손 인식 완료' : '오른손 인식 대기'}</Text>
                <Text style={styles.recognitionDetail} numberOfLines={2}>{cameraStatus}</Text>
              </View>
              <Text style={styles.frameCount}>{acceptedFrames}/{capturedFrames}</Text>
            </View>
            ...
            >
              {!coachingAllowed ? (
                <View style={[styles.feedbackPanel, styles.feedbackNeutral]}>
                  <Text style={styles.feedbackEyebrow}>판정 대기</Text>
                  <Text style={styles.feedbackTitle}>{waitingMessage}</Text>
                  <Text style={styles.feedbackBody}>손이나 자세가 확인되기 전에는 간격·자세·점수·경고를 만들지 않습니다.</Text>
                </View>
              ) : trajectory ? (
...
const styles = StyleSheet.create({
  ...
  practiceRoot: { flex: 1, backgroundColor: '#0d1117', paddingBottom: Platform.OS === 'android' ? 16 : 8 },
  ...
  header: { minHeight: 52, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: '#30363d', backgroundColor: '#161b22' },
  headerCopy: { flex: 1 },
  ...
  closeButton: { minWidth: 62, minHeight: 40, borderRadius: 11, borderWidth: 1, borderColor: '#6e7681', backgroundColor: '#21262d', alignItems: 'center', justifyContent: 'center' },
  closeButtonText: { color: '#ffffff', fontSize: 11, fontWeight: '900' },

  modeRow: { minHeight: 48, flexDirection: 'row', gap: 6, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: '#0d1117' },
  modeButton: { flex: 1, minHeight: 36, borderRadius: 10, borderWidth: 1, borderColor: '#30363d', backgroundColor: '#161b22', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 },
  ...
  routineRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 11, paddingVertical: 4, borderTopWidth: 1, borderTopColor: '#21262d', borderBottomWidth: 1, borderBottomColor: '#21262d', backgroundColor: '#11161d' },
  routineCopy: { flex: 1, paddingRight: 8 },
  routineLabel: { color: '#8b949e', fontSize: 7, fontWeight: '900' },
  routineTitle: { color: '#ffffff', fontSize: 11, fontWeight: '900', marginTop: 2 },
  routineButton: { minWidth: 78, minHeight: 32, borderRadius: 9, borderWidth: 1, borderColor: '#6e7681', backgroundColor: '#21262d', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 8 },
  routineButtonText: { color: '#ffffff', fontSize: 8, fontWeight: '900' },

  primaryControlRow: { minHeight: 68, flexDirection: 'row', gap: 8, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: '#0d1117' },
  ...
  startButtonSub: { color: '#e6edf3', fontSize: 7, fontWeight: '700', marginTop: 3, textAlign: 'center' },
  bpmBox: { width: 116, minHeight: 54, borderRadius: 14, borderWidth: 1, borderColor: '#30363d', backgroundColor: '#161b22', flexDirection: 'row', alignItems: 'center', overflow: 'hidden' },
  bpmAdjust: { width: 34, alignSelf: 'stretch', alignItems: 'center', justifyContent: 'center', backgroundColor: '#21262d' },
  bpmAdjustText: { color: '#ffffff', fontSize: 22, fontWeight: '700' },
  bpmCopy: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  bpmNumber: { color: '#ffffff', fontSize: 18, fontWeight: '900' },
  bpmUnit: { color: '#8b949e', fontSize: 6, fontWeight: '900' },

  cameraFrame: { width: '100%', alignSelf: 'center', backgroundColor: '#000000', borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#30363d', overflow: 'hidden' },
  recognitionStrip: { minHeight: 48, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 11, paddingVertical: 6, backgroundColor: '#161b22', borderBottomWidth: 1, borderBottomColor: '#30363d' },
  statusDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#9e6a03', marginRight: 9 },
  statusDotReady: { backgroundColor: '#2ea043' },
  recognitionCopy: { flex: 1 },
  recognitionTitle: { color: '#ffffff', fontSize: 10, fontWeight: '900' },
  recognitionDetail: { color: '#b1bac4', fontSize: 8, lineHeight: 12, marginTop: 2 },
  frameCount: { color: '#79c0ff', fontSize: 9, fontWeight: '900', marginLeft: 8 },

  feedbackScroll: { flex: 1, minHeight: 0, backgroundColor: '#0d1117' },
  feedbackContent: { padding: 10, paddingBottom: Platform.OS === 'android' ? 34 : 20 },
  feedbackPanel: { borderRadius: 14, borderWidth: 1, padding: 13 },
  feedbackNeutral: { backgroundColor: '#111d2f', borderColor: '#1f6feb' },
  feedbackGood: { backgroundColor: '#13251a', borderColor: '#2ea043' },
  feedbackBad: { backgroundColor: '#2b1718', borderColor: '#da3633' },
  ...
  sectionLabel: { color: '#f2cc60', fontSize: 8, fontWeight: '900', marginTop: 11 },
  feedbackBody: { color: '#d8dee4', fontSize: 10, lineHeight: 16, marginTop: 3 },
  metricsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 12 },
  metric: { color: '#ffffff', backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 6, fontSize: 8, fontWeight: '900', overflow: 'hidden' },
  errorText: { color: '#ffb4ad', fontSize: 9, lineHeight: 14, textAlign: 'center', marginTop: 9 },
  savedText: { color: '#7ee787', fontSize: 9, lineHeight: 14, textAlign: 'center', marginTop: 9 },
